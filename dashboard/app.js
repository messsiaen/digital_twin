...
async function refresh() {
  try {
    ...
    elStamp.textContent = `Updated: ${new Date(m.ts).toLocaleString()}`;

    const series = m.series || {};
    rainChart.setOption(lineOpts('Rain (mm/h)', series.rain || []));
    sinrChart.setOption(lineOpts('SINR (dB)',   series.sinr || []));
    dutyChart.setOption(lineOpts('Duty Cycle',  series.duty || [], '{value}'));
    mosChart.setOption (lineOpts('MOS',         series.mos  || [], '{value}'));
  } catch (e) {
    elStamp.textContent = 'Data fetch failed. Will retry…';
